...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            origen_ant = place.ant
            if origen_ant.can_contain(self):
                origen_ant.store_ant(self)
            elif self.can_contain(origen_ant):
                self.store_ant(origen_ant)
                place.ant = self
            else:
    ...
    def buff(self):
        ...
        if not self.is_buffed:
            self.is_buffed = True
            self.damage = self.__class__.damage * 2
...
class ThrowerAnt(Ant):
    ...
    # END Problem 1
    # BEGIN Problem 4
    thrower_ant_type = None
    min_range = 0
    max_range = 0x3f3f3f3f
    # END Problem 4
    ...
    def nearest_bee(self):
        ...
        nearest_place:Place = self.place
        cur_range = 0
        while (not nearest_place.is_hive) and cur_range <= self.max_range:
            if self.min_range <= cur_range and nearest_place.bees:
                return random_bee(nearest_place.bees)
            nearest_place,cur_range = nearest_place.entrance,cur_range+1
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # ant_damage = (amount + FireAnt.damage) if will_die else amount
        # bees_copy = self.place.bees.copy()
        # bees_left = [bee for bee in bees_copy if bee.health > ant_damage]
        # for bee in bees_copy:
        #     Insect.reduce_health(bee,ant_damage)
        # Ant.reduce_health(self,amount)
        # self.place.bees = bees_left
        ...
        will_die = (self.health <= amount)
        ant_damage = (amount + self.damage) if will_die else amount
        bees_copy = this_place.bees.copy()
        for bee in bees_copy:
            bee.reduce_health(ant_damage)
...
class WallAnt(Ant):
    """
    WallAnt can stop the bees, but does not attack bees.
    """
    name = 'Wall'
    food_cost = 4
    # OVERRIDE CLASS ATTRIBUTES HERE
...
class HungryAnt(Ant):
    """
    HungryAnt can stop the bees, but does not attack bees.
    """
    name = 'Hungry'
    chew_duration = 3
    food_cost = 4
    ...
    def action(self,gamestate):
        """Eat a random bee in its place if chew_cntdown is 0, otherwise reduce c_cd by 1"""
        if self.chew_cntdown <= 0:
            target = random_bee(self.place.bees)
            if target is not None:
                self.eat_bee(target)
                self.chew_cntdown = self.chew_duration
        else:
            self.chew()
# END Problem 7
...
class TankAnt(ContainerAnt):
    "TankAnt provides protection to other Ants, and deals 1 damage to all bees in its place each turn"
    name = "Tank"
    food_cost = 6
    damage = 1
    # OVERRIDE CLASS ATTRIBUTES HERE
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem Optional 1
        ant = self.place.ant
        return ant is not None and ant.is_blocks_path
        # END Problem Optional 1
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem EC
        destination = self.place.exit if self.status["scared"] <= 0 else self.place.entrance
        
        # Extra credit: Special handling for bee direction
        if self.blocked():
            self.sting(self.place.ant)
        elif self.health > 0 and destination is not None:
            if self.status["slowed"] > 0 and gamestate.time % 2 == 1:
                self.status["slowed"] -= 1
            else:
                self.move_to(destination)
                self.status["scared"] = max(self.status["scared"]-1,0)
        # END Problem EC
    ...
    def scare(self, length):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if not self.anti_scared:
            self.status["scared"] += length
            self.anti_scared = True
        # END Problem EC
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem Optional 1
        this_place = self.place
        bees_copy = list(this_place.bees)
        for bee in bees_copy:
            bee.reduce_health(self.damage)
        # END Problem Optional 1
